
    
    customElements.define('my-search-chart-image-dynamicdata',
      class MyComponent extends HTMLElement{
        template = `<div style="display:flex;flex-direction:column;padding-top:10px;padding-right:10px;padding-bottom:10px;padding-left:10px;width:100%;"><div style="display:flex;flex-direction:row;padding-top:10px;padding-right:10px;padding-bottom:10px;padding-left:10px;width:100%;"><my-text-6875087274029696 _data="_ngElementStrategy.componentRef.instance"
                         _methods="_ngElementStrategy.componentRef.instance" 
                        style="font-size:14px;color:black;"></my-text-6875087274029696><my-input-394920017609085 _data="_ngElementStrategy.componentRef.instance" _methods="_ngElementStrategy.componentRef.instance"
                    type="text" 
               ></my-input-394920017609085><my-text-5823204118626595 _data="_ngElementStrategy.componentRef.instance"
                         _methods="_ngElementStrategy.componentRef.instance" 
                        style="font-size:14px;color:black;"></my-text-5823204118626595><my-radio-5623554309288894 _data="_ngElementStrategy.componentRef.instance" _methods="_ngElementStrategy.componentRef.instance"></my-radio-5623554309288894><my-text-8896720377487346 _data="_ngElementStrategy.componentRef.instance"
                         _methods="_ngElementStrategy.componentRef.instance" 
                        style="font-size:14px;color:black;"></my-text-8896720377487346><my-select-9512372760572303 _data="_ngElementStrategy.componentRef.instance" _methods="_ngElementStrategy.componentRef.instance"></my-select-9512372760572303><my-icon-01444185709940582 _data="_ngElementStrategy.componentRef.instance" 
                        _methods="_ngElementStrategy.componentRef.instance"></my-icon-01444185709940582><my-button-19604536985253063 _data="_ngElementStrategy.componentRef.instance" _methods="_ngElementStrategy.componentRef.instance"></my-button-19604536985253063></div><div style="padding-top:10px;padding-right:10px;padding-bottom:10px;padding-left:10px;width:100%;"><my-table-9244400052572201 _data="_ngElementStrategy.componentRef.instance" _methods="_ngElementStrategy.componentRef.instance"></my-table-9244400052572201></div><div style="padding-top:10px;padding-right:10px;padding-bottom:10px;padding-left:10px;width:100%;"><line-chart-8407966617635663></line-chart-8407966617635663></div><div style="padding-top:10px;padding-right:10px;padding-bottom:10px;padding-left:10px;width:100%;"><my-image-7051406588239237 _data="_ngElementStrategy.componentRef.instance"
                        _methods="_ngElementStrategy.componentRef.instance" 
                       style="classes:undefined;style:undefined;"></my-image-7051406588239237></div></div><my-api-5901175525267992 _data="_ngElementStrategy.componentRef.instance" _methods="_ngElementStrategy.componentRef.instance"></my-api-5901175525267992>`;
        constructor(){
          super();
          this.innerHTML = this.template;
        }
      }
    );
    class MyText6875087274029696 extends TextComponent{
              constructor(){
                  super();
                  this.text = '姓名'
                  this.dep();
              }
              dep(){
                setTimeout(()=>{
                  this.cd = this['__ngContext__'][13][0]._ngElementStrategy.componentRef.changeDetectorRef;
                });
              }
          }
          MyText6875087274029696.ɵcmp.factory = () => { return new MyText6875087274029696()};
          customElements.define('my-text-6875087274029696',createCustomElement(MyText6875087274029696, {  injector: injector,}));
          class MyInput394920017609085 extends InputComponent{
            constructor(){
                super();
                this.placeholder="请输入姓名"
                this.control.reset('',{
                  updateOn:''
                });
                
                this.control.updateValueAndValidity();
                this.control.statusChanges.subscribe((res)=>{
                  if(res === 'VALID'){
                    this.validateTrue.emit();
                  }else{
                    this.validateFalse.emit();
                  }
                });
                this.dep();
             }
             get value() {
               return {name:this.control.value};
             }
             set value(target) {
               this.control.setValue(target);
             }
            dep(){
             setTimeout(()=>{
               this.cd = this['__ngContext__'][13][0]._ngElementStrategy.componentRef.changeDetectorRef;
             });
            }
        };
        MyInput394920017609085.ɵcmp.factory = () => { return new MyInput394920017609085()};
        customElements.define('my-input-394920017609085',createCustomElement(MyInput394920017609085, {  injector: injector,}));
        class MyText5823204118626595 extends TextComponent{
              constructor(){
                  super();
                  this.text = '性别'
                  this.dep();
              }
              dep(){
                setTimeout(()=>{
                  this.cd = this['__ngContext__'][13][0]._ngElementStrategy.componentRef.changeDetectorRef;
                });
              }
          }
          MyText5823204118626595.ɵcmp.factory = () => { return new MyText5823204118626595()};
          customElements.define('my-text-5823204118626595',createCustomElement(MyText5823204118626595, {  injector: injector,}));
          class MyRadio5623554309288894 extends RadioComponent{
            constructor(){
                super();
                this.options = [{"label":"男","value":"男","checked":true},{"label":"女","value":"女","checked":false}];
                this.control = '男';
            }
            get value() {
              return {sex:this.control};
            }
            set value(target) {
              this.control = target;
            }
        }
        MyRadio5623554309288894.ɵcmp.factory = () => { return new MyRadio5623554309288894()};
        customElements.define('my-radio-5623554309288894',createCustomElement(MyRadio5623554309288894, {  injector: injector}));
        class MyText8896720377487346 extends TextComponent{
              constructor(){
                  super();
                  this.text = '朋友'
                  this.dep();
              }
              dep(){
                setTimeout(()=>{
                  this.cd = this['__ngContext__'][13][0]._ngElementStrategy.componentRef.changeDetectorRef;
                });
              }
          }
          MyText8896720377487346.ɵcmp.factory = () => { return new MyText8896720377487346()};
          customElements.define('my-text-8896720377487346',createCustomElement(MyText8896720377487346, {  injector: injector,}));
          class MySelect9512372760572303 extends SelectComponent{
            constructor(){
                super(SelectComponent.copyHttp);
                this.options = [{"label":"男","value":"男","checked":false},{"label":"女","value":"女","checked":false}];
                this.selected = '';
                this.api = 'https://api.randomuser.me/?results=10';
                this.placeholder = '请选择';
                console.log(this.http);
                
            }
            get value() {
              return {friend:this.selected};
            }
            set value(target) {
              this.selected = target;
            }
        }
        MySelect9512372760572303.ɵcmp.factory = () => { return new MySelect9512372760572303()};
        customElements.define('my-select-9512372760572303',createCustomElement(MySelect9512372760572303, {  injector: injector}));
        class MyIcon01444185709940582 extends IconComponent{
             constructor(){
                 super();
                 this['fontSize'] = '16px'
this['icon'] = '#icon-tubiao'
             }
         }
         MyIcon01444185709940582.ɵcmp.factory = () => { return new MyIcon01444185709940582()};
         customElements.define('my-icon-01444185709940582',createCustomElement(MyIcon01444185709940582, {  injector: injector}));
         class MyButton19604536985253063 extends ButtonComponent{
               constructor(){
                   super();
                   this.disabled = false
this.ghost = false
this.loading = false
this.shape = ''
this.size = 'default'
this.type = 'primary'
this.block = false
this.danger = false
this.icon = 'search'
this.name = 'search'
                   this.dep();
                }
                dep(){
                  setTimeout(()=>{
                    this.cd = this['__ngContext__'][13][0]._ngElementStrategy.componentRef.changeDetectorRef;
                  });
                }
           }
           MyButton19604536985253063.ɵcmp.factory = () => { return new MyButton19604536985253063()};
           customElements.define('my-button-19604536985253063',createCustomElement(MyButton19604536985253063, {  injector: injector,}));
           class MyTable9244400052572201 extends TableComponent{
             constructor(){
                 super();
                 this.bordered = false
this.loading = false
this.pagination = false
this.sizeChanger = false
this.title = false
this.header = true
this.footer = false
this.expandable = false
this.checkbox = false
this.fixHeader = false
this.noResult = false
this.ellipsis = false
this.simple = false
this.size = 'default'
this.tableScroll = 'unset'
this.tableLayout = 'auto'
this.position = 'top'
this.headers = [{"label":"Name:100","key":"Name:100"},{"label":"Age:100","key":"Age:100"},{"label":"Address","key":"Address"}]
this.row = undefined;
                 this.dep();
             }
             dep(){
              setTimeout(()=>{
                this.cd = this['__ngContext__'][13][0]._ngElementStrategy.componentRef.changeDetectorRef;
              });
             }
             // 配置项
             get list() {
               return this.listOfData;
             }
             set list(data) {
               this.listOfData = data || [];
               this.check();
             }
         }
         MyTable9244400052572201.ɵcmp.factory = () => { return new MyTable9244400052572201()};
         customElements.define('my-table-9244400052572201',createCustomElement(MyTable9244400052572201, {  injector: injector,}));
         class LineChart8407966617635663 extends LineChartComponent{
                    that;
                    constructor(){
                        super();
                        setTimeout(()=>{
                            let index = Object.keys(this).filter(key=>key.startsWith('__reactContainer'));
                            let ins = this.that = this[index[0]].child.stateNode;
                            ins.applyData({title : '',
width : '400px',
height : '200px',
xData : ["Mon","Tue","Wed","Thu","Fri","Sat","Sun"],
series : [{"name":"销量","type":"line","data":[5,20,36,10,10,20,0]},{"name":"销量2","type":"line","data":[15,30,46,20,20,30,0]}],})
                        },300);
                    }
                    get config(){
                        console.log('config');
                    }
                    set config(value){
                        console.log('value',value)
                        const {title,xData,series} = value || {};
                        this.that.applyData({title,xData,series});
                    }   
                };
                customElements.define('line-chart-8407966617635663',LineChart8407966617635663);
                class MyImage7051406588239237 extends ImageComponent{
             constructor(){
                 super();
                 this.width = 'auto'
this.height = 'auto'
this.src = 'data:image/webp;base64,'
             }
         }
         MyImage7051406588239237.ɵcmp.factory = () => { return new MyImage7051406588239237()};
         customElements.define('my-image-7051406588239237',createCustomElement(MyImage7051406588239237, {  injector: injector,}));
         class MyAPI5901175525267992 extends RequestComponent{
               constructor(){
                   super(RequestComponent.httpCopy);
                   this.method = 'get'
this.api = 'https://www.fastmock.site/mock/14c2723aefa052a75b2a6feeed0cf387/suger/records'
               }
           }
           MyAPI5901175525267992.ɵcmp.factory = () => { return new MyAPI5901175525267992()};
           customElements.define('my-api-5901175525267992',createCustomElement(MyAPI5901175525267992, {  injector: injector,}));
           ;
            (()=>{
                const sourceDOM = document.querySelector('my-button-19604536985253063'),
                    targetDOM = document.querySelector('my-api-5901175525267992'),
                    targetPath = (targetDOM.getAttribute('_methods')||'').split('.');
                sourceDOM.addEventListener('click', (e)=>{
                  let targetIns = targetPath.length ? targetPath.reduce((pre,key)=>key ? pre[key] : pre,targetDOM) : targetDOM;
                  targetIns['request']();
                });
            })();
              
            (()=>{
              const sourceDOM = document.querySelector('my-api-5901175525267992'),
                    targetDOM = document.querySelector('my-table-9244400052572201'),
                    sourcePath = (sourceDOM.getAttribute('_data')||'').split('.'),
                    targetPath = (targetDOM.getAttribute('_data')||'').split('.');
              sourceDOM.addEventListener('success200',()=>{
                let sourceIns = sourcePath.length ? sourcePath.reduce((pre,key)=>key ? pre[key] : pre,sourceDOM) : sourceDOM,
                    targetIns = targetPath.length ? targetPath.reduce((pre,key)=>key ? pre[key] : pre,targetDOM) : targetDOM;
                targetIns['list'] = sourceIns['list'];
              })
            })();
          
          (()=>{
            const sourceDOM = document.querySelector('my-radio-5623554309288894'),
                  targetDOM = document.querySelector('my-api-5901175525267992'),
                  sourcePath = (sourceDOM.getAttribute('_data')||'').split('.'),
                  targetPath = (targetDOM.getAttribute('_methods')||'').split('.');
            let sourceIns = sourcePath.length ? sourcePath.reduce((pre,key)=>key ? pre[key] : pre,sourceDOM) : sourceDOM,
                targetIns = targetPath.length ? targetPath.reduce((pre,key)=>key ? pre[key] : pre,targetDOM) : targetDOM;   
            if(!targetIns['request'].params){
              targetIns['request'].params = []
            }
            targetIns['request'].params.push([sourceIns,["value"]]);
          })();  
          
          (()=>{
            const sourceDOM = document.querySelector('my-select-9512372760572303'),
                  targetDOM = document.querySelector('my-api-5901175525267992'),
                  sourcePath = (sourceDOM.getAttribute('_data')||'').split('.'),
                  targetPath = (targetDOM.getAttribute('_methods')||'').split('.');
            let sourceIns = sourcePath.length ? sourcePath.reduce((pre,key)=>key ? pre[key] : pre,sourceDOM) : sourceDOM,
                targetIns = targetPath.length ? targetPath.reduce((pre,key)=>key ? pre[key] : pre,targetDOM) : targetDOM;   
            if(!targetIns['request'].params){
              targetIns['request'].params = []
            }
            targetIns['request'].params.push([sourceIns,["value"]]);
          })();  
          
          (()=>{
            const sourceDOM = document.querySelector('my-input-394920017609085'),
                  targetDOM = document.querySelector('my-api-5901175525267992'),
                  sourcePath = (sourceDOM.getAttribute('_data')||'').split('.'),
                  targetPath = (targetDOM.getAttribute('_methods')||'').split('.');
            let sourceIns = sourcePath.length ? sourcePath.reduce((pre,key)=>key ? pre[key] : pre,sourceDOM) : sourceDOM,
                targetIns = targetPath.length ? targetPath.reduce((pre,key)=>key ? pre[key] : pre,targetDOM) : targetDOM;   
            if(!targetIns['request'].params){
              targetIns['request'].params = []
            }
            targetIns['request'].params.push([sourceIns,["value"]]);
          })();  
          
            (()=>{
              const sourceDOM = document.querySelector('my-api-5901175525267992'),
                    targetDOM = document.querySelector('line-chart-8407966617635663'),
                    sourcePath = (sourceDOM.getAttribute('_data')||'').split('.'),
                    targetPath = (targetDOM.getAttribute('_data')||'').split('.');
              sourceDOM.addEventListener('success200',()=>{
                let sourceIns = sourcePath.length ? sourcePath.reduce((pre,key)=>key ? pre[key] : pre,sourceDOM) : sourceDOM,
                    targetIns = targetPath.length ? targetPath.reduce((pre,key)=>key ? pre[key] : pre,targetDOM) : targetDOM;
                targetIns['config'] = sourceIns['lineChart'];
              })
            })();
          
    